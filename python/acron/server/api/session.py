...
from acron.server.utils import dump_args, check_ldap_group_membership
from acron.server.http import generate_http_payload
...
class User(UserMixin):
    ''' flask-login User class implementation '''
...
def verify_yubicode(username, yubicode):
    ...
    try:
        try:
            ...
            logging.error(
                "Config Error: Yubicode validation url is not defined")
            return 0
...
def login():
    ...
    logging.debug("username: %s endpoint: %s url: %s",
                  username, request.endpoint, request.url)
    if username is None:
        return "Username is not defined. Login failed"
...
def login_status():
    ...
    username = request.remote_user
    status = False
    if not check_ldap_group_membership(username, ldap_group=current_app.config["USERS_GROUP"]):
        return generate_http_payload(status_code=401,
                                     message=f'Please subscribe your user, {username} ' +
                                     f', to the LDAP group {current_app.config["USERS_GROUP"]}')
